...
3. The directory/file system that you can operate is the logical path `/workspace/`. DO NOT try to save/read/modify file in other directories.
...
11. When you use the `write_file` tool, you **MUST ALWAYS** remember to provide the both the `path` and `content` parameters. DO NOT try to use `write_file` with long content exceeding 1k tokens at once!!!
