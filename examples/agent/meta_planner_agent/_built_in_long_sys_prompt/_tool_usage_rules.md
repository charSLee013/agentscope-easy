### Tool usage rules
1. When using online search tools, provide a concise keyword `query` (2-6 words) and use the optional `context` field to summarize the goal or constraints for that search. Avoid pasting raw content into the search request.
