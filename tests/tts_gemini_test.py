...
class GeminiTTSModelTest(IsolatedAsyncioTestCase):
    ...
    async def test_synthesize_returns_empty_for_none_or_empty_text(self) -> None:
        ...
        model._client.models.generate_content.reset_mock()
        ...
        model._client.models.generate_content.assert_not_called()

    async def test_synthesize_returns_empty_audio_when_response_has_no_audio(
        self,
    ) -> None:
        """Missing inline audio should degrade to an empty audio block."""
        model = GeminiTTSModel(
            api_key=self.api_key,
            stream=False,
        )
