        ...
        **_kwargs: Any,
        ...
        Args:
            ...
            multimodality (`bool | None`, optional):
                Whether to force the multimodal conversation API. If `None`,
                the route is inferred from the model name.
        ...
        if headers:
            try:
                headers_obj = json.loads(str(headers))
                if not isinstance(headers_obj, dict):
                    raise json.JSONDecodeError("", "", 0)

                if self.generate_kwargs.get("headers"):
                    headers_obj.update(self.generate_kwargs["headers"])

                self.generate_kwargs["headers"] = headers_obj
    ...
    async def __call__(
        ...
        **kwargs: Any,
        ...
        if self.multimodality or (
            ...
            and (self.model_name.startswith("qvq") or "-vl" in self.model_name)
        ):
